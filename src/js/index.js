...
const generateEmptyTable = () => {
    TABLE.innerHTML = '';
    for (const i of _.range(1, MAX_ROW_SIZE + 1)) {
        for (const j of _.range(1, MAX_COLUMN_SIZE + 1)) {
...
const isLeftToRightAvailable = (row, from, to) => {
    for (let i = from; i <= to; i++) {
        const cell = document.querySelector('.row' + row + '.col' + i);
...
const setWordLeftToRight = word => {
    const minimunSize = MAX_COLUMN_SIZE - word.length + 1;
    ...
    for (let i = 0; i < word.length; i++) {
        const cell = document.querySelector('.row' + row + '.col' + (column + i));
...
const setWordUpToDown = word => {
    const minimunSize = MAX_ROW_SIZE - word.length + 1;
    ...
    // console.log("Encontrado hueco para " + word + " en: (" + row + ", " + column + ")");
    for (let i = 0; i < word.length; i++) {
        const cell = document.querySelector('.row' + (row + i) + '.col' + column);
        cell.textContent = word[i];
        cell.classList.add('marked');
    }
};
...
const checkWord = e => {
    const checked = document.querySelectorAll('.soup_table div.checked');
    if (checked.length === 0) {
